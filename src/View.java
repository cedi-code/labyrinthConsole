...
/**
 * Die Klasse View stellt ein Fenster auf dem Bildschirm zur Verf�gung, in welchem
...
public class View extends JFrame {
  ...
  private List<Kachel> figuren = new ArrayList<Kachel>();
  private Labyrinth kacheln;

  /**
   * Konstruktor. Initialisiert das Fenster in der Mitte des Bildschirms und erzeugt ein
   * JFrame-Objekt, auf welchem die Figuren gezeichnet werden.
   */
  public View(Labyrinth k, int w, int h) {
    this.kacheln = k;
    Dimension windowSize = new Dimension(w*50, h*50);
    setSize(windowSize);
  ...
  private void zeichneFiguren(Graphics g) {
    for (Kachel k : kacheln.getLabyrinthfeld()) {
        k.render(g);
    }
  }

  public void setFiguren(int p, Kachel k) {
      figuren.set(p,k);
      repaint();
  }
   ...
   */
  public void hinzufuegen(Kachel figur) {
    figuren.add(figur);
    repaint();
  }
